...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        files = []
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]: #相对路径转为绝对路径
            ...
            if '*' in p:
                ...
                files.append(p)  # files,["C:\\Users\\zkh\\Desktop\\yolov5\\data\\images\\bus.jpg"]
            else:
                raise FileNotFoundError(f'{p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS] #图片格式文件路径
