...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Times New Roman.ttf', pil=False, example='abc'):
...
def get_one_box(xyxy, im, gain=1.00, pad=0, square=False, BGR=False):
    # Save image crop as {file} with crop size multiple {gain} and {pad} pixels. Save and/or return crop
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    if square:
