...
class Hungarian():
    ...
    # val1 = ids[match[i][0]]
    # val2 = previousindex[match[i][1]]
    # str(Hungarian.idx)+str(val1) + str(val2)
    def runHungarian(self,detections,tracks: list = None,previousindex: dict = None):

        #这是第一帧
        ids = {}
        two_ids = {}  #融合id
        if tracks is None:
           Hungarian.idx = len(detections)
           return {i:i for i in range(0,len(detections))}
        else:
           match,undeter,untrack = self.associate_detections_to_trackers(detections,tracks)
           print("--------match-------")
           print(match)
           for i,ret in enumerate(match):
               #如果这个ids[xx] 存在值,你又要赋值了 说明存在二合一问题
               if ids.get(match[i][0]) != None:
                   two_ids[Hungarian.idx] = "("+str(ids.get(match[i][0]))+","+str(previousindex[match[i][1]])+")"
                   ids[match[i][0]] = Hungarian.idx
                   Hungarian.idx = Hungarian.idx + 1
                   print("------------------------")
                   print(two_ids)
               else:
                   ids[match[i][0]] = previousindex[match[i][1]]
           for j,ret in enumerate(undeter):
               ids[undeter[j]] = Hungarian.idx
               Hungarian.idx = Hungarian.idx + 1
           #排个序
           ids = dict(sorted(ids.items(), key=operator.itemgetter(0)))
           return ids,two_ids
    ...
    def Area_batch(self,bb_test, bb_gt):
        """
        在两个box间计算IOU
        :param bb_test: box1 = [x1y1x2y2] 即 [左上角的x坐标，左上角的y坐标，右下角的x坐标，右下角的y坐标]
        :param bb_gt: box2 = [x1y1x2y2]
    ...
    def associate_detections_to_trackers(self,detections, trackers, iou_threshold=0.2):
        ...
        for d, det in enumerate(detections):
            ...
            for t, trk in enumerate(trackers):
                ...
                行索引值对应的是当前目标检测框。列索引值对应的是前一帧目标检测框。
                """
                iou_matrix[d, t] = self.Area_batch(det, trk)
                if iou_matrix[d,t] >= 0.4:
                    list_check.append((d,t))
        #print(iou_matrix)
        ...
        matched_indices = np.array(res_list)
        ...
        return matches, np.array(sorted(unmatched_detections)), np.array(unmatched_trackers)
